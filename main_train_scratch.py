...
def run(input_sizes,learning_rate,epochs,batch,node,workers,imnet_path,architecture,shared_folder_path,job_id,local_rank,global_rank,num_tasks):
    ...
    abs_path = os.getcwd()
    shared_folder_path = os.path.join(abs_path, shared_folder_path)
    imnet_path = os.path.join(abs_path, imnet_path)
    ...
    if Path(str(imnet_path)).is_dir():
        ...
        raise RuntimeError("No imagenet data folder available")
